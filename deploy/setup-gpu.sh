...
echo "[EXTRA] Installing training dependencies..."
pip install unsloth trl datasets 2>/dev/null || echo "unsloth install may need manual setup"

# --- Install Claude Code ---
echo "[EXTRA] Installing Claude Code CLI..."
npm install -g @anthropic-ai/claude-code 2>/dev/null || echo "Claude Code install needs API key setup"
...
echo "=========================================="
echo ""
echo "STEP 1 — Start the self-improvement loop:"
echo "  cd /root/ollama-team"
echo "  uvicorn backend.main:app --host 0.0.0.0 --port 8000 &"
echo "  npx serve frontend/dist -l 5173 &"
echo "  # Dashboard: http://<YOUR_IP>:5173"
echo ""
echo "STEP 2 — Extract training data from all your repos:"
echo "  python training/extract_training_data.py"
echo ""
echo "STEP 3 — Fine-tune your personal model:"
echo "  python training/fine_tune.py"
echo ""
echo "STEP 4 — Load into Ollama:"
echo "  ollama create dylan-coder -f training/output/Modelfile"
echo "  ollama run dylan-coder"
echo ""
echo "STEP 5 — Copy model home when done:"
echo "  scp training/output/gguf_export/*.gguf dylan@IRONPC2:~/"
echo ""
echo "BONUS — Run Claude Code on this machine:"
echo "  export ANTHROPIC_API_KEY=your-key-here"
echo "  claude"
echo ""
echo "Models: qwen2.5-coder:32b | Context: 16K | Cooldown: 15s"
